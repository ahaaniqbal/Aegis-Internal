 ...
 *
 * Backend logic preserved: same /auth/validate-reset-token GET on mount,
 * same POST to /auth/reset-password, same client-side strength check.
 */
...
// Feature bullets shown in the brand-showcase panel — kept narrow because
// "reset password" is a single-purpose page, the showcase should remind
// users why they're signing in to Aegis without competing for attention.
...
function ResetPasswordPage() {
  ...
  // ─── Validate token on mount (LOGIC UNCHANGED) ─────────────────────────
  useEffect(() => {
  ...
  // ─── Layout shell — split-screen, matches /auth ─────────────────────────
  return (
...
function SuccessState() {
  return (
    <motion.div variants={fadeUp} className="text-center">
      ...
      <Link
        href="/auth"
        className="mt-6 inline-flex items-center justify-center rounded-[8px] bg-[var(--primary-base)] px-4 py-2.5 text-[13px] font-semibold text-white shadow-[inset_0_1px_0_0_rgba(255,255,255,0.18),0_1px_2px_rgba(206,94,18,0.30)] [background:linear-gradient(180deg,#fb8939_0%,#fa7319_55%,#ed6a14_100%)] border border-[var(--primary-dark)] transition-all hover:[background:linear-gradient(180deg,#fa7d27_0%,#ed6a14_55%,#d75e10_100%)]"
      >
        Go to log in
      </Link>
    </motion.div>
  ...
  return (
    <>
      ...
      >
        ...
        {/* Password strength checklist — polished, brand-cohesive callout */}
